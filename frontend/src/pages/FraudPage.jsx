...
export default function FraudPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setResult({ error: e.message || 'Failed to analyze data' });
    }
    setLoading(false);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="glass-panel">
      <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
        <ShieldAlert size={40} className="glow-text-cyan" />
        <div>
          <h1 style={{ margin: 0, fontSize: '2.2rem' }}>Bank Fraud Detection</h1>
          <p style={{ margin: 0, color: 'var(--text-muted)' }}>Dense AutoEncoder Sequence</p>
        </div>
      </div>
      
      <div style={{ marginTop: '30px' }}>
        <p style={{ color: 'var(--text-muted)', marginBottom: '10px' }}>INPUT_TENSOR [n_features]:</p>
        <textarea 
          className="cyber-input"
          value={data}
          onChange={(e) => setData(e.target.value)}
          placeholder="0.001, -2.435, 1.221, 0.443... (Enter numerical array)"
          rows={4}
        />
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
          <button className="cyber-button" onClick={handleAnalyze} disabled={loading || !data.trim()}>
            {loading ? <><Activity size={18} style={{ animation: 'spin 2s linear infinite' }} /> PROCESSING...</> : <><ChevronRight size={18} /> INJECT PAYLOAD</>}
          </button>
        </div>
      </div>
